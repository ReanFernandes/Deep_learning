...
def create_2unit_net() -> Module:
    ...
    w1 = np.array([[3.21, -2.34], [3.21, -2.34]])
    b1 = np.array([-3.21, 2.34])
    w2 = np.array([[3.19, -2.68], [4.64, -3.44]])
    b2 = np.array([-4.08, 4.42])
    model.parameters()[0].data = w1
    model.parameters()[1].data = b1
    model.parameters()[2].data = w2
    model.parameters()[3].data = b2
    # END TODO ##################
...
def run_model_on_xor(model: Module, verbose: bool = True) -> Tuple[np.ndarray, float]:
    ...
    loss = loss_fn.forward(prediction, Y_onehot)
    # END TODO ##################
